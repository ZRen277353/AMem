...
bool loadModules(Mem::IMemService& service,
...
ModulesWindow::ModulesWindow(Mem::IMemService& memService)
...
void ModulesWindow::onDraw()
{
	...
	{
		std::vector<Mem::ModuleInfo> list;
		Mem::Error error;
		if (loadModules(memService_, list, &error)) {
			hasData = true;
			modules = std::move(list);
			Gui::log("获取到 %d 个模块", (int)modules.size());
		} else {
			hasData = false;
			Gui::log("获取模块列表失败 [%s]: %s",
			         Mem::errorCodeName(error.code), error.message.c_str());
		}
	}
	
	ImGui::SameLine();
	const auto current = memService_.captureContext(true);
	int pid = current.target ? current.target->pid : 0;
	if (pid)
	...
	if (autoRefreshOnce) {
		...
		std::vector<Mem::ModuleInfo> list;
		if (loadModules(memService_, list)) {
			hasData = true;
			modules = std::move(list);
		}
	}
			...
			for (const auto* modulePtr : filteredModules) {
				...
				if (ImGui::BeginPopupContextItem(popup_id)) {
					...
					if (ImGui::MenuItem("在内存查看器中打开")) {
						navigateToAddress(module.base, memService_);
