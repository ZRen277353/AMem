...
Every in-app tool is `ToolSafety::ReadOnly` or `ToolSafety::Write`. Non-Lua write tools require confirmation unless `autoApproveWrites` is enabled. `lua_execute` is controlled independently by `autoApproveLuaExecution`, defaults to automatic approval, and requires confirmation when that setting is disabled; the general write switch never overrides the Lua setting.
...
`IpcMethodCatalog` contains the same 24 canonical names as the in-app registry: 12 Observe, 1 TargetSelection, 9 TargetMutation, and 2 HostExecution; target policies are 3 None, 19 Bound, and 2 Selection (`process_open` and `lua_execute`). Only the 12 Observe entries bypass the broker. `IpcMemServiceDispatcher` reuses `MemJsonTools`, captures `{connectionGeneration, target}` once per external session, checks it before/after dispatch and at 250 ms reader intervals, and propagates the same absolute deadline and atomic cancellation token into `OperationContext`. A stale baseline emits session-level Error, cancels active work, and joins. A dispatcher without a bound broker/session still returns `approval_required`. The product runtime binds both: privileged requests submit only method/client/session/target/deadline metadata, consume one durable grant, then parse and execute the active request. The shared Lua setting may make a `lua_execute` record immediately approved, but it does not bypass submission, context validation, durable one-shot consumption, or audit, and the broker rejects policy auto-approval for every other method. A Lua script may call `process.attach`; only the service-confirmed final snapshot may advance the script, Agent run, and IPC session baselines. Eleven privileged methods use `MemJsonTools`/`MemService`; `lua_execute` uses an injected `IIpcHostMethodExecutor` backed by shared `Mem::executeLuaJson()` when LuaJIT is available. Cancellation after consume is checked before the adapter/send boundary.

`NativeAgentRuntime` owns the `NamedPipeServer` and composes each accepted handle as framed connection -> handshake -> `IpcMemServiceDispatcher` -> request session. Each successful Hello receives a server-assigned monotonic session id; the snapshot exposes current/last ids alongside lifecycle/client identity/capability/status/counters and clears active client identity after completion. Session ids are not reset across stop/start. The system approval audit outlives the broker, host executor, and runtime; every established session exit cancels approvals bound to that exact id. Request params and results are not retained by runtime/broker/audit/GUI. Stop joins the server handler and request worker. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds still start with no listener and require a user click to enable `Observe + privileged policy approval`. Hello never grants a standing privileged capability.

`IpcApprovalBroker` defines the privileged authorization core. It derives capability and target policy from `IpcMethodCatalog`, rejects Observe/unknown/unbounded submissions, stores no params/results, bounds live/history records, and permits pending -> approved -> consumed or terminal deny/invalidate/expire/cancel transitions. Its second submission mode may create an approved record only for `lua_execute`; every other method is rejected if code attempts policy auto-approval. `consume()` revalidates session, request, deadline, connection generation, and target under the broker mutex. It burns the record before the unlocked audit call and returns a one-shot grant only if the consumed transition is durable; audit failure returns `approval_audit_failed`, leaves the record consumed, and cannot be retried. A concurrent session Cancel and consume have one terminal winner. After consume, `IpcMemServiceDispatcher` synchronously records one `execution_outcome` with the `auto_approved` marker, authorized/observed snapshots, success, completion, and bounded error code. Post-effect audit failure remains visible but must not rewrite a confirmed or uncertain device receipt. Static gates require Lua-only policy approval, dispatcher consumption, grant-bound execution/outcome audit, a shared Lua target recheck, and Observe-only Hello.
...
19. Keep native IPC compile-time default-off and runtime default-stopped; only explicit GUI action may enable `Observe + privileged policy approval`. Hello must not grant standing privileged capability; every privileged execution requires a fresh target-bound, durable one-shot grant. Lua policy may skip the GUI decision only for `lua_execute`, never broker submission, consumption, or audit. All handlers and workers stay owned and joinable.
