...
`native_agent_mem_service` is the main no-device C++ test with 23 groups. Native IPC has 7 approval-broker, 5 protocol, 5 transport, 7 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 5 `MemService` dispatcher, and 9 owned-runtime groups. They cover bounded approval DTO/state/retention/audit callbacks, one-shot grants, target/generation/session/stop invalidation, framing/transport, Hello, strict JSON, lifetime request ids, Observe dispatch, sequential clients, monotonic server session ids, precise session-bound approval cancellation, cancellation, and joined shutdown. Fifteen CTests include those ten native IPC binaries plus the service suite and four static gates. Provider, broker submission/dispatch/persistent-audit integration, live GUI interaction, opt-in legacy IPC, real Android transport, and device operations still lack complete automation. For manual wire-protocol checks use `tools/protocol_reference/amem_client.py`; for the live Lua API use `scripts/dump_api.lua` as described in `scripts/README.md`. Changes involving real device state, concurrency, cancellation, or teardown still need manual end-to-end verification with the GUI and an Android device.
...
`NativeAgentRuntime` owns the `NamedPipeServer` and composes each accepted handle as framed connection -> handshake -> `IpcMemServiceDispatcher` -> request session. Each successful Hello receives a server-assigned monotonic session id; the snapshot exposes current/last ids alongside lifecycle/client identity/capability/status/counters and clears active client identity after completion. Session ids are not reset across stop/start. The system broker outlives the runtime, and every established session exit cancels approvals bound to that exact id. Request params and results are never retained. Stop signals the pipe, cancels handshake or active dispatch, and joins the server handler and request worker. `ENABLE_NATIVE_IPC` compiles an explicit `NativeAgentIpcWindow`; it defaults OFF, and compiled builds still start with no listener. The window lazily creates the system runtime, shows status/counters, and requires a user click to enable Observe. `main.cpp` only performs shutdown before device disconnect. Privileged request submission, capability grants, and dispatcher consume integration remain unimplemented, so do not describe native IPC as a privileged external Agent adapter.

`IpcApprovalBroker` defines the privileged authorization core. It derives capability and target policy from `IpcMethodCatalog`, rejects Observe/unknown/unbounded submissions, stores no params/results, bounds live/history records, and permits only pending -> approved -> consumed or terminal deny/invalidate/expire/cancel transitions. Approval is not an execution grant until a one-shot `consume()` revalidates connection generation and target. The system owner now holds the broker; `NativeAgentIpcWindow` lists bounded pending records and offers approve/deny, while Stop/shutdown cancels all live records. Runtime session exit also calls `cancelSession()` with the server id, covering disconnect and target/session invalidation without broad cancellation. Static gates forbid params/results in this UI, forbid runtime submission, and keep Hello Observe-only. No request path submits or consumes an approval yet; direct privileged dispatch still returns `approval_required`.
