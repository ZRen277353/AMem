...
The supported AI integration is the in-app AI Chat agent in `gui/ai/`. It supports Claude, OpenAI-compatible, and DeepSeek providers and calls native debugger tools. The Python MCP proxy and its IDE configurations have been removed from `NativeAgent`. Default builds exclude the old loopback HTTP IPC server; `ENABLE_LEGACY_HTTP_IPC=ON` restores it only for migration diagnostics. Native framing, secured transport, strict Hello/request sessions, a complete method catalog, 12 Observe `MemService` adapters, an owned runtime, explicit status/control, and a bounded privileged-approval UI/state machine now exist under `ipc/` and `gui/`. Established pipe sessions receive server-assigned monotonic ids that are not reused across runtime restart; session close, target invalidation, and Stop cancel approvals bound to that id. A privileged request may now submit bounded metadata from the owned request worker while the reader remains available for Cancel. Approval still returns `approval_execution_disabled`: there is no consume/send-boundary execution or persistent audit, Hello remains Observe-only, and no privileged external Agent adapter is enabled. `ENABLE_NATIVE_IPC` still defaults OFF; even when compiled, the pipe starts disabled and requires a user action.
...
`native_agent_mem_service` is the main no-device C++ test with 23 groups. Native IPC has 8 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 6 `MemService` dispatcher, and 10 owned-runtime groups. They cover bounded approval DTO/state/retention/audit callbacks, exact request/session cancellation, one-shot grants, target/generation/session/stop invalidation, framing across polling deadlines, bounded terminal Error drainage, Hello, strict JSON, lifetime request ids, Observe dispatch, non-executing privileged submission, monotonic server session ids, cancellation, and joined shutdown. Fifteen CTests include those ten native IPC binaries plus the service suite and four static gates. Provider, privileged consume/send-boundary execution, persistent-audit integration, live GUI interaction, opt-in legacy IPC, real Android transport, and device operations still lack complete automation. For manual wire-protocol checks use `tools/protocol_reference/amem_client.py`; for the live Lua API use `scripts/dump_api.lua` as described in `scripts/README.md`. Changes involving real device state, concurrency, cancellation, or teardown still need manual end-to-end verification with the GUI and an Android device.
...
`IpcMethodCatalog` contains the same 24 canonical names as the in-app registry: 12 Observe, 1 TargetSelection, 9 TargetMutation, and 2 HostExecution; target policies are 3 None, 20 Bound, and 1 Selection. Only the 12 Observe entries are executable without approval. `IpcMemServiceDispatcher` reuses `MemJsonTools` for those methods, captures `{connectionGeneration, target}` once per external session, checks it before/after dispatch and at 250 ms reader intervals, and propagates the same absolute deadline and atomic cancellation token into `OperationContext`. A stale baseline emits session-level Error, cancels active work, and joins. A dispatcher without a bound broker/session still returns `approval_required`. The product runtime binds both, so privileged requests submit only method/client/session/target/deadline metadata and wait on the owned worker; raw params remain in the active request and never reach `MemJsonTools` or `MemService` in this slice.

`NativeAgentRuntime` owns the `NamedPipeServer` and composes each accepted handle as framed connection -> handshake -> `IpcMemServiceDispatcher` -> request session. Each successful Hello receives a server-assigned monotonic session id; the snapshot exposes current/last ids alongside lifecycle/client identity/capability/status/counters and clears active client identity after completion. Session ids are not reset across stop/start. The system broker outlives the runtime, and every established session exit cancels approvals bound to that exact id. Request params and results are never retained by runtime/broker/GUI. Stop signals the pipe, cancels handshake or active dispatch, and joins the server handler and request worker. `ENABLE_NATIVE_IPC` compiles an explicit `NativeAgentIpcWindow`; it defaults OFF, and compiled builds still start with no listener. The window lazily creates the system runtime, shows status/counters, and requires a user click to enable Observe. `main.cpp` only performs shutdown before device disconnect. Privileged submission is wired, but capability grants, persistent audit, and dispatcher consume/send-boundary execution remain unimplemented, so do not describe native IPC as a privileged external Agent adapter.

`IpcApprovalBroker` defines the privileged authorization core. It derives capability and target policy from `IpcMethodCatalog`, rejects Observe/unknown/unbounded submissions, stores no params/results, bounds live/history records, and permits only pending -> approved -> consumed or terminal deny/invalidate/expire/cancel transitions. Approval is not an execution grant until a one-shot `consume()` revalidates connection generation and target. The request worker now submits against the exact server session/request id; client Cancel calls exact `cancelRequest()`, while session exit calls `cancelSession()`. `NativeAgentIpcWindow` lists bounded pending records and offers approve/deny, and Stop/shutdown cancels all live records. Static gates forbid params/results in this UI, forbid runtime submission and dispatcher consumption, and keep Hello Observe-only. Approved requests are cancelled and returned as `approval_execution_disabled`; no request is consumed or executed yet.
...
21. Treat IPC approval as a one-shot, target-bound authorization: never retain raw params/results in broker records, revalidate again at consume/send, invalidate on session/target/generation changes, and never equate an approved record with execution.
