...
`native_agent_mem_service` is the main no-device C++ test with 23 groups. Native IPC has 4 approval-audit, 8 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 6 `MemService` dispatcher, and 10 owned-runtime groups. Coverage includes bounded JSONL persistence/rotation/reload/failure status, exact request/session cancellation, one-shot grants, framing across polling deadlines, terminal Error drainage, Observe dispatch, non-executing privileged submission, monotonic session ids, cancellation, and joined shutdown. Sixteen CTests include those eleven native IPC binaries plus the service suite and four static gates. Provider, privileged consume/send-boundary execution, live GUI click interaction, opt-in legacy IPC, real Android transport, and device operations still lack complete automation. For manual wire-protocol checks use `tools/protocol_reference/amem_client.py`; for the live Lua API use `scripts/dump_api.lua` as described in `scripts/README.md`. Changes involving real device state, concurrency, cancellation, or teardown still need manual end-to-end verification with the GUI and an Android device.
...
`NativeAgentRuntime` owns the `NamedPipeServer` and composes each accepted handle as framed connection -> handshake -> `IpcMemServiceDispatcher` -> request session. Each successful Hello receives a server-assigned monotonic session id; the snapshot exposes current/last ids alongside lifecycle/client identity/capability/status/counters and clears active client identity after completion. Session ids are not reset across stop/start. The system approval audit outlives the broker, which outlives runtime; every established session exit cancels approvals bound to that exact id. Request params and results are never retained by runtime/broker/audit/GUI. Stop joins the server handler and request worker. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds still start with no listener and require a user click to enable Observe. Privileged submission and persistent transition audit are wired, but capability grants and dispatcher consume/send-boundary execution remain unimplemented, so do not describe native IPC as a privileged external Agent adapter.

`IpcApprovalBroker` defines the privileged authorization core. It derives capability and target policy from `IpcMethodCatalog`, rejects Observe/unknown/unbounded submissions, stores no params/results, bounds live/history records, and permits only pending -> approved -> consumed or terminal deny/invalidate/expire/cancel transitions. Approval is not an execution grant until a one-shot `consume()` revalidates connection generation and target. The request worker submits against the exact server session/request id; request/session/Stop cancellation is bounded. `IpcApprovalAuditLog` receives transitions outside the broker mutex, writes a bounded JSONL plus one backup, reloads only bounded valid lines, and exposes write failures. Audit failure currently does not roll back a decision; the next consume slice must fail closed before returning a grant. Static gates forbid params/results in audit/UI, forbid dispatcher consumption, and keep Hello Observe-only. Approved requests still return `approval_execution_disabled`.
