...
3. submission、session/request cancel 与 persistent audit 已完成；下一步让 `consume()` 在审计失败时烧毁授权且不返回 grant，再接 send-time target recheck 与真实 executor。完成前不得启用 privileged grant。
