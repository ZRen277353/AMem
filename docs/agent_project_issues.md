...
审计范围：`gui/ai/`、`mem/MemJsonTools.*`、`ipc/IpcProtocol.*`、`ipc/IpcFramedConnection.*`、`ipc/IpcHandshakeSession.*`、`ipc/IpcRequestProtocol.*`、`ipc/IpcRequestSession.*`、`ipc/IpcMethodCatalog.*`、`ipc/IpcMemServiceDispatcher.*`、`ipc/NamedPipeServer.*`、`ipc/NativePipeSecurity.*`、`ipc/IpcServer.*`、`socket/` 中被 Agent/IPC 调用的命令层、`gui/AppContext.*`、`main.cpp`。
...
- Native transport 已有 protected 当前用户/SYSTEM read-write DACL、`PIPE_REJECT_REMOTE_CLIENTS`、单实例所有权、有界 framed I/O、Observe-only Hello、严格 request session、完整 24-name catalog、12 个 Observe `MemService` adapter 和 connection/target baseline invalidation；但 `ENABLE_NATIVE_IPC` 默认 OFF、main 无启动路径，也没有 runtime composition、GUI status 或 privileged approval broker。
...
2. 若确认需要外部自动化，在已落地的 transport/session/Observe adapter 上实现 owned runtime composition 与 GUI enable/status，而不是恢复 Python MCP。
...
- 新 `NamedPipeServer` 不使用 detached handler：accept 与串行 handler 在一个 owned thread 上，stop event + `CancelIoEx` 后 join；但它当前没有产品 start 路径，也未替代 legacy HTTP。
...
仓库已有 `NativeAgentMemTests`/`native_agent_mem_service` 的 23 个测试组，覆盖地址/scalar codec、driver receipt/card redaction、进程与模块分页/解析、事务化 pointer resolution、disassembly、scan/symbol session/full-table transaction、breakpoint receipt/rich hit batch、scan 取消/完成未知、mutation audit 脱敏/轮转/晚到 callback 前写盘、target/generation、raw/typed write 完成语义、连接 lease/poison、审批失效、同批 target 推进、非目标工具、排队取消/timeout、active cancellation、shutdown join、晚到结果拒绝和 33 个退役工具的历史降级。Native IPC 有 5 组 protocol、5 组 transport、7 组 framed-I/O、8 组 handshake、6 组 request-contract、9 组 request-session、4 组 method-catalog 和 5 组 MemService-dispatcher 测试，覆盖 session invalidation、完整 capability/target classification、12 Observe mapping、privileged direct-call denial、error/retryable normalization 和 service context propagation；`native_agent_native_ipc_gate` 固定 compile default-off/no-main-start。Debug/Release 当前各有 13 项 CTest。以下纯逻辑/协议边界仍缺自动化：
...
- Legacy IPC HTTP parser/partial send，以及 Native IPC runtime composition、GUI lifecycle、privileged approval 与 target-bound approval invalidation。
...
1. 在保持 HTTP IPC 与 native runtime default-off 的前提下，把已固定 transport/Hello/session/catalog/Observe adapter 装配进 owned runtime，并实现 GUI 状态与 privileged approval broker；完成前不能建立产品 start 路径，也不能默认开启旧端口。
