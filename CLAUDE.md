...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 7 approval-broker, 5 protocol, 5 transport, 7 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 5 `MemService` dispatcher, and 9 runtime groups; the total suite has fifteen CTests. Coverage includes bounded approval state/retention/audit callbacks, one-shot target-bound grants, stop-time cancel-all, framing/transport, Hello, strict JSON, catalog classification, Observe dispatch, connection/target invalidation, sequential sessions, monotonic server session ids, exact session-bound approval cancellation, cancellation, and joined shutdown. Provider, broker submission/dispatch/persistent-audit integration, live GUI interaction, opt-in legacy IPC, real Android transport, and device paths still need coverage. For manual wire-protocol checks, use `tools/protocol_reference/amem_client.py` or dump the live Lua API surface with `scripts/dump_api.lua` (see `scripts/README.md`).
...
`NativeAgentRuntime` owns the server and composes framed connection -> Hello -> Observe dispatcher -> request session for each serial client. A successful Hello gets a monotonic server session id that is not reset by stop/start. Its thread-safe snapshot contains current/last session ids, lifecycle, active client identity/capabilities, bounded status/counters, and errors, but never request params or results. Every established session exit cancels broker approvals for that id; Stop joins both handler and dispatch worker. `ENABLE_NATIVE_IPC` defaults OFF. When compiled, `NativeAgentIpcWindow` exposes an explicit enable/stop action and visible Observe-only status; the system runtime is constructed lazily, begins stopped, and is shut down before device disconnect. There is still no wired privileged request path, and handshake continues to deny every privileged capability.

`IpcApprovalBroker` is an authorization core, not an enabled capability path. Catalog metadata is server-owned; approval records exclude params/results and are bounded. Pending approvals may be approved or terminate as denied/invalidated/expired/cancelled. An approved record remains revocable and yields a one-shot grant only after `consume()` revalidates generation/target. The system owner constructs it before the runtime and destroys it afterward; the Native IPC window can display/decide pending records, Stop/shutdown cancels all live approvals, and session exit precisely cancels records for its server id. No production request submits or consumes a record yet, so Hello still grants only Observe and privileged dispatcher calls still fail with `approval_required`.
