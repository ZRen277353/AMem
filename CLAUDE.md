...
On top of the GUI it ships an **in-app AI chat agent** (multi-provider: Claude / OpenAI / DeepSeek) that drives the debugger through native tool calls. The Python MCP proxy has been removed. Default builds exclude the loopback HTTP IPC server. Native framing, secured transport, strict Hello/request sessions, a complete method catalog, 12 Observe adapters, an owned runtime, explicit control, and a bounded privileged approval state machine exist under `ipc/` and `gui/`. Privileged metadata can enter the broker while the reader handles Cancel, and transitions use a bounded plaintext JSONL audit with visible failure status. Broker `consume()` now burns authorization and withholds the grant if consumed audit is not durable. Product requests still return `approval_execution_disabled`: Hello grants only Observe and dispatcher/send-boundary execution is absent. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds start with the pipe stopped.
...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 5 approval-audit, 12 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 6 dispatcher, and 10 runtime groups; the total suite has sixteen CTests. Approval coverage includes JSONL bounds/failures, durable grant withholding, identity/context checks, one-shot burning, and consume/Cancel races. Privileged dispatcher/send-boundary execution, live GUI interaction, opt-in legacy IPC, real Android transport, and device paths still need coverage.
...
`IpcApprovalBroker` is an authorization core, not an enabled capability path. Catalog metadata is server-owned; records exclude params/results and are bounded. `consume()` revalidates session/request/deadline/generation/target, burns the record before unlocked audit, and returns a grant only for a durable consumed transition. Audit failure returns `approval_audit_failed` with no grant and no reusable authorization; consume and session Cancel are linearized by the broker state. Submit/decide/cancel audit failures remain locally visible without rolling back their safe outcome. No production path calls `consume()`; approved requests return `approval_execution_disabled`, Hello grants only Observe, and no privileged device/host call occurs.
