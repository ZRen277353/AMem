...
On top of the GUI it ships an **in-app AI chat agent** (multi-provider: Claude / OpenAI / DeepSeek) that drives the debugger through native tool calls. The Python MCP proxy has been removed. Default builds exclude the loopback HTTP IPC server; `ENABLE_LEGACY_HTTP_IPC=ON` restores that unsafe endpoint only for migration diagnostics. Native framing, secured transport, strict Hello/request sessions, a complete method catalog, 12 Observe `MemService` adapters, an owned runtime, explicit status/control, and an isolated privileged approval state machine exist under `ipc/` and `gui/`. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds still start with the pipe stopped and require a user click to enable Observe. The broker is not wired to Hello, dispatch, GUI decisions, or persistent audit, so no privileged external Agent adapter exists yet.
...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 6 approval-broker, 5 protocol, 5 transport, 7 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 5 `MemService` dispatcher, and 7 runtime groups; the total suite has fifteen CTests. Coverage includes bounded approval state/retention/audit callbacks, one-shot target-bound grants, framing/transport, Hello, strict JSON, catalog classification, Observe dispatch, connection/target invalidation, sequential sessions, cancellation, and joined shutdown. Provider, broker GUI/dispatch/persistent-audit integration, opt-in legacy IPC, real Android transport, and device paths still need coverage. For manual wire-protocol checks, use `tools/protocol_reference/amem_client.py` or dump the live Lua API surface with `scripts/dump_api.lua` (see `scripts/README.md`).
...
`IpcApprovalBroker` is an isolated authorization core, not an enabled capability path. Catalog metadata is server-owned; approval records exclude params/results and are bounded. Pending approvals may be approved or terminate as denied/invalidated/expired/cancelled. An approved record remains revocable and yields a one-shot grant only after `consume()` revalidates generation/target. Its audit sink runs outside the broker mutex. No production component currently constructs the broker, so Hello still grants only Observe and privileged dispatcher calls still fail with `approval_required`.
