...
On top of the GUI it ships an **in-app AI chat agent** (multi-provider: Claude / OpenAI / DeepSeek) that drives the debugger through native tool calls. The Python MCP proxy has been removed. Default builds exclude the loopback HTTP IPC server; `ENABLE_LEGACY_HTTP_IPC=ON` restores that unsafe endpoint only for migration diagnostics. Native framing, secured transport, strict Hello/request sessions, a complete method catalog, 12 Observe `MemService` adapters, an owned runtime, explicit status/control, and a bounded privileged approval UI/state machine exist under `ipc/` and `gui/`. Successful Hello sessions receive monotonic server ids that survive runtime restart; closing or invalidating a session cancels approvals bound to that id. Privileged request metadata can now enter the broker from the owned worker while the reader handles Cancel, but approval returns `approval_execution_disabled`: Hello still grants only Observe, and consume/send-boundary execution plus persistent audit are absent. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds still start with the pipe stopped and require a user click to enable Observe.
...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 8 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 6 `MemService` dispatcher, and 10 runtime groups; the total suite has fifteen CTests. Coverage includes bounded approval state/retention/audit callbacks, exact request/session cancellation, one-shot target-bound grants, stop-time cancel-all, framing across polling deadlines, bounded terminal Error drainage, Hello, strict JSON, catalog classification, Observe dispatch, non-executing privileged submission, connection/target invalidation, monotonic session ids, cancellation, and joined shutdown. Provider, privileged consume/send-boundary execution, persistent audit, live GUI interaction, opt-in legacy IPC, real Android transport, and device paths still need coverage. For manual wire-protocol checks, use `tools/protocol_reference/amem_client.py` or dump the live Lua API surface with `scripts/dump_api.lua` (see `scripts/README.md`).
...
`IpcMethodCatalog` mirrors all 24 canonical names and fixes capability plus target policy; only 12 Observe methods are approval-free. `IpcMemServiceDispatcher` reuses `MemJsonTools`, binds the external session to its initial connection/target snapshot, polls and rechecks that baseline, and propagates deadline/cancellation into `OperationContext`. Privileged requests submit bounded metadata and wait on the owned worker while the reader remains available for Cancel; they still cannot reach parameter adapters or service invocation.

`NativeAgentRuntime` owns the server and composes framed connection -> Hello -> dispatcher -> request session for each serial client. A successful Hello gets a monotonic server session id that is not reset by stop/start. Its thread-safe snapshot contains current/last session ids, lifecycle, active client identity/capabilities, bounded status/counters, and errors, but never request params or results. Every established session exit cancels broker approvals for that id; Stop joins both handler and dispatch worker. `ENABLE_NATIVE_IPC` defaults OFF. When compiled, `NativeAgentIpcWindow` exposes an explicit enable/stop action and visible Observe-only status; the system runtime is constructed lazily, begins stopped, and is shut down before device disconnect. Privileged requests may be submitted for a decision but cannot execute, and handshake continues to deny every privileged capability.

`IpcApprovalBroker` is an authorization core, not an enabled capability path. Catalog metadata is server-owned; approval records exclude params/results and are bounded. Pending approvals may be approved or terminate as denied/invalidated/expired/cancelled. An approved record remains revocable and yields a one-shot grant only after `consume()` revalidates generation/target. The system owner constructs it before the runtime and destroys it afterward; the request worker submits exact session/request metadata, client Cancel revokes that request, the window can decide it, and session/Stop shutdown cancellation remains bounded. No production path calls `consume()`; approved requests return `approval_execution_disabled`, Hello grants only Observe, and no privileged device/host call occurs.
