...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 6 security-audit, 12 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 15 dispatcher, and 10 runtime groups. Socket coverage adds 4 client-transport and 6 multi-port manager groups for real Winsock loopback, scripted partial I/O/failures, all-port rollback, request/disconnect exclusion, and reconnect isolation. Provider stream coverage adds 14 parser/state-machine groups, and persistence recovery adds 4 groups for corrupt-index recovery plus transactional API-key, settings, and session loads. The total suite has twenty CTests. Live provider HTTP, GUI approval clicks, real Android transport, and device paths still need coverage.
