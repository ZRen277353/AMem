...
On top of the GUI it ships an **in-app AI chat agent** (multi-provider: Claude / OpenAI / DeepSeek) that drives the debugger through native tool calls. The Python MCP proxy has been removed. Default builds exclude the loopback HTTP IPC server. Native framing, secured transport, strict Hello/request sessions, a complete method catalog, 12 Observe adapters, an owned runtime, explicit control, and a bounded privileged approval state machine exist under `ipc/` and `gui/`. Privileged metadata can enter the broker while the reader handles Cancel, and every transition is written to a bounded plaintext JSONL audit with visible failure status. Approval still returns `approval_execution_disabled`: Hello grants only Observe and consume/send-boundary execution is absent. `ENABLE_NATIVE_IPC` defaults OFF; compiled builds start with the pipe stopped.
...
`native_agent_mem_service` contains 23 no-device C++ groups. Native IPC adds 4 approval-audit, 8 approval-broker, 5 protocol, 5 transport, 8 framed-I/O, 8 handshake, 6 request-contract, 9 request-session, 4 method-catalog, 6 dispatcher, and 10 runtime groups; the total suite has sixteen CTests. Audit coverage includes JSONL fields, rotation/reload, malformed and oversized lines, 200 concurrent writes, and visible disk failures. Privileged consume/send-boundary execution, live GUI interaction, opt-in legacy IPC, real Android transport, and device paths still need coverage.
...
`IpcApprovalBroker` is an authorization core, not an enabled capability path. Catalog metadata is server-owned; records exclude params/results and are bounded. `IpcApprovalAuditLog` outlives the broker, persists every transition outside the broker mutex, reloads bounded valid JSONL lines, and reports failures to the control window. Sink calls now return durability status for the next fail-closed consume slice; current decisions are not rolled back on audit failure. No production path calls `consume()`; approved requests return `approval_execution_disabled`, Hello grants only Observe, and no privileged device/host call occurs.
